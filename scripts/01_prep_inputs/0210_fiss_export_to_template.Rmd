...
```{r params}
# dplyr is used unqualified below (bind_rows, mutate, ...). Attach it rather than
# leaning on a warm session - the "self-sufficient in a clean environment" claim
# below only holds if the script attaches what it uses. Same pattern as
# `scripts/03_permit_submission/fds_prep_for_submission.R`. `scripts/packages.R`
# is deliberately not sourced: it reaches out to CRAN on every run.
suppressMessages({
  library(dplyr)
})
...
#
# Render from the repo root WITH knit_root_dir, e.g.
#   rmarkdown::render("scripts/01_prep_inputs/0210_fiss_export_to_template.Rmd",
#                     knit_root_dir = getwd())
# knitr otherwise moves the working directory to the Rmd's own folder and none of
# the repo-relative paths below resolve.
