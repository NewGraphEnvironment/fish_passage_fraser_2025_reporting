# fish_passage_fraser_2025_reporting 0.6.0 (2026-08-06)
...
* Promote the statistical modelling content out of a fourth-level heading buried under Planning and into its own Methods section, matching the Peace report and the template. The heading no longer names `bcfishpass` alone, because the work now supports our own habitat modelling as well as the provincial outputs.
* Move the collaborative GIS environment section to the end of the methods chapter, matching the Peace report and the template. It had been sitting second, ahead of the modelling and field methods it supports; the results chapter already had it last.
* Drop the issue-tracker links from the water temperature attachment, and point its remaining links at `NewGraphEnvironment` rather than `poissonconsulting`.
* State what the program is for, in the executive summary and in the recommendations. Each field and modelling activity was described on its own, leaving the reader to infer why they belong in one report. Both chapters now open by saying that the components are designed to complement one another, building the state of knowledge regarding watershed health and directing restoration effort to where it will do the most good — then name what each contributes. Passage assessments and the provincial model establish where fish can reach; habitat confirmations, fish sampling and environmental DNA establish where they are; floodplain delineation and aerial imagery establish how infrastructure has altered the valley bottom; climate departure and water temperature establish the trajectory; effectiveness monitoring tests whether the work delivered. The summary also states that making the evidence usable — structured field forms, methods documented beside the code that implements them, layers published to open catalogues, reporting written to be questioned — is part of the work rather than a by-product.
* Remove the generic territorial acknowledgement sentence from the front matter.
