...
**Warming is broad, fast, and significant region-wide.** All eight ecoregions of the study area warmed about `r sprintf("%.1f", tmean_d)` °C since 1951 — annual mean (+`r sprintf("%.1f", tmean_d)` °C), daytime maximum (+`r sprintf("%.1f", tmax_d)` °C), and overnight minimum (+`r sprintf("%.1f", tmin_d)` °C) all tell the same story. The trend is highly significant (Mann-Kendall p < 0.001 in every ecoregion). A north-west-to-south-east gradient layers on top, with the interior plateau ecoregions warming roughly 0.5 °C more than the eastern Rocky Mountain ecoregions — see [Appendix - Climate Departure](#app-climate-departure) Figure \@ref(fig:cd-map-tmean).
...
**The snowpack signal is about timing, not quantity.** Total annual snowfall is essentially unchanged (`r sprintf("%+.0f", snowfall_p)` %); annual snow water equivalent is down `r sprintf("%.0f", -swe_p)` %, neither significant. But the seasonal redistribution is sharp — summer SWE collapsed `r sprintf("%.0f", -summer_swe_p)` %, winter snowmelt rose `r sprintf("%+.0f", winter_melt_p)` %, spring snowmelt rose `r sprintf("%+.0f", spring_melt_p)` %, and the snowmelt midpoint shifted `r sprintf("%.0f", -doy50_d)` days earlier (p < 0.001 regionally and in every ecoregion). The freshet is arriving weeks earlier and the high-elevation snowpack that historically lingered into summer no longer does.
