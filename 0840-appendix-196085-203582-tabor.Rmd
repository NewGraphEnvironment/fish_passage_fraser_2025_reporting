...
```{r map-196085, fig.cap = my_caption, fig.width = 9, fig.height = 7, dpi = 150, out.width = photo_width}
...
Detection results are presented in Table \@ref(tab:tab-edna-results-196085). Format is `CODE(max_droplets)`, with `*` flagging samples UNBC reran. Detected means four or more positive droplets, the call threshold applied by the laboratory; results below that threshold are reported as not detected. Site-level detail for the whole project, along with field blanks and retests, is in [Appendix - Environmental DNA Results](#app-edna).
